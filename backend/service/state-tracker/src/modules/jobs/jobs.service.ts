...
export const jobsService = {
  ...
  async startJob(jobId: string): Promise<Job> {
    ...
    eventsService
      ...
      .catch((err) =>
        console.error("[events] JOB_STARTED append failed:", err),
      );
  ...
  async startStep(
    ...
    eventsService
      ...
      .catch((err) =>
        console.error("[events] STEP_STARTED append failed:", err),
      );
  ...
  async completeStep(
    ...
    job: Job;
    step: import("../steps/steps.types").JobStep | null;
  }> {
    const job = await jobsRepository.findById(jobId);
    if (!job) throw new NotFoundError("Job", jobId);
  ...
  async failStep(
    ...
    eventsService
      ...
      .catch((err) => console.error("[events] JOB_FAILED append failed:", err));
  ...
  async getJobState(
    ...
    if (eventsOptions?.includeEvents !== false) {
      const { events, total } = await eventsService.getByJobId(
        jobId,
        eventsOptions,
      );
