...
export function registerScheduledJobs() {
  ...
  cron.schedule("0 * * * *", async () => {
    ...
    try {
      const hasHeadroom = await hasRateLimitHeadroom(
        config.INGESTION_BATCH_SIZE * 80,
      );
      if (!hasHeadroom) {
        console.warn(
          "[CRON] Not enough GitHub rate limit headroom. Skipping tick.",
        );
        return;
      }
  ...
  cron.schedule("0 0 * * 0", async () => {
    ...
    try {
      ...
      console.info(
        `[CRON] Enqueueing weekly reports for ${tracked.length} developers.`,
      );
