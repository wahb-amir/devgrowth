...
assert(
  near(sig(0, 60), 0.018, 0.005),
  `sig(0, k) = 0.018 floor [got ${sig(0, 60).toFixed(3)}]`,
);
assert(
  near(sig(60, 60), 0.5, 0.005),
  `sig(k, k) = 0.5 midpoint [got ${sig(60, 60).toFixed(3)}]`,
);
assert(
  sig(180, 60) > 0.97,
  `sig(3k, k) > 0.97 elite [got ${sig(180, 60).toFixed(3)}]`,
);
...
assert(
  norm730 < norm365,
  "Older account has smaller normFactor (less inflation)",
);
assert(
  norm365 / norm730 < 1.5,
  "Sub-linear: doubling age raises factor by <50%",
);
...
assert(near(tCritical95(30), 1.96, 0.1), "t-critical(n≥30) ≈ 1.96");
...
assert(
  results["ghost"]!.finalScore < 30,
  `Ghost in weak band [got ${results["ghost"]!.finalScore}]`,
);
assert(
  results["spammer"]!.finalScore < 50,
  `Spammer penalised below 50 [got ${results["spammer"]!.finalScore}]`,
);
assert(
  results["maintainer"]!.finalScore >= 45,
  `Maintainer in average+ band [got ${results["maintainer"]!.finalScore}]`,
);
assert(
  results["elite"]!.finalScore >= 75,
  `Elite in strong+ band [got ${results["elite"]!.finalScore}]`,
);
...
assert(
  results["spammer"]!.antiExploit.lowSubstanceCommits,
  "Spammer: low-substance commits flagged",
);
assert(
  results["spammer"]!.antiExploit.singleRepoConcentration,
  "Spammer: single-repo concentration flagged",
);
assert(!results["elite"]!.antiExploit.spamFlagged, "Elite: no spam flags");
assert(
  !results["maintainer"]!.antiExploit.spamFlagged,
  "Maintainer: no spam flags",
);
...
assert(
  results["ghost"]!.archetype === "ghost" ||
    results["ghost"]!.archetype === "balanced",
...
assert(
  typeof eliteNarrative.trajectoryStatement === "string",
  "Elite: trajectory statement",
);
assert(
  typeof eliteNarrative.confidenceStatement === "string",
  "Elite: confidence statement",
);

// Ghost should have watch areas
assert(
  results["ghost"]!.narrative.watchAreas.length >= 0,
  "Ghost: watch areas (may be empty for ghost)",
);
...
assert(
  typeof yourResult.narrative.headline === "string",
  "Your dev: headline generated",
);

// ── Confidence & CI assertions ─────────────────────────────────
console.log("\n▸ Confidence & Uncertainty");

const newDev = scoreV3Full({
  snapshot: REFERENCE_DATASET["newcomer"]!,
  history: [],
  snapshotCount: 1,
});
...
assert(
  newDev.confidenceLevel === "very_low" || newDev.confidenceLevel === "low",
  `New dev: low confidence [got ${newDev.confidenceLevel}]`,
);
assert(
  veteran.confidenceLevel === "high" || veteran.confidenceLevel === "very_high",
...
assert(
  newWidth > vetWidth,
  `New dev CI wider than veteran [${newWidth.toFixed(1)} vs ${vetWidth.toFixed(1)}]`,
);
...
assert(
  rising.trend > 0,
  `Rising history → positive trend [got ${rising.trend}]`,
);
assert(
  declining.trend < 0,
  `Declining history → negative trend [got ${declining.trend}]`,
);
...
console.log(
  `  Same activity: young (180d) = ${youngDev.finalScore}, old (1800d) = ${oldDev.finalScore}`,
);
assert(
...
const zeroResult = scoreV3Full({
  snapshot: REFERENCE_DATASET["ghost"]!,
  history: [],
});
assert(!isNaN(zeroResult.finalScore), "All-zero: no NaN");
assert(
  zeroResult.finalScore >= 0 && zeroResult.finalScore <= 100,
  "All-zero: in [0,100]",
);

// Mega values
const megaSnap = makeSnapshot({
  pushes: 99999,
  prs: 9999,
  stars: 999999,
  followers: 999999,
...
const megaResult = scoreV3Full({
  snapshot: megaSnap,
  history: [],
  snapshotCount: 25,
});
assert(
  megaResult.finalScore <= 100,
  `Mega values clamped [got ${megaResult.finalScore}]`,
);
